...
Art: dev/icon/build_scene.js rendered with VM.camera(0.14) -> banner_frames/ (1600px, 1:1, no resampling).
...
W, H = 1536, 512
FPS = 25
STILL_FRAME = 11  # runes in flight from the book to the pickaxe, vein intact
ART_OFFSET = (372, -530)  # banner = frame + offset: the loop's extent centred on x 1180 and vertically
OUTLINE = 17
LAYERS = (("banner_bg", 8, (0, 0)), ("banner_title_top", 6, (74, 111)), ("banner_title", 8, (72, 207)),
          ("banner_tagline", 6, (74, 347)))
...
def main() -> None:
    ...
    if not files:
        sys.exit("no frames in dev/icon/banner_frames - render them from Blockbench first (VM.camera(0.14))")
